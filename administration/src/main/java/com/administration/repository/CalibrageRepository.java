...
public interface CalibrageRepository extends JpaRepository<Calibrage, Long> {
	...
	@Query("SELECT DISTINCT c FROM Calibrage c WHERE c.actif = 1 AND c.ratio IS NOT NULL AND c.ratio.id=:idRatio AND "
			+ "((c.min IS NOT NULL AND :valeurRatio > c.min "
			+ "AND c.max IS NOT NULL AND :valeurRatio <= c.max) "
			+ "OR (c.min IS NULL AND :valeurRatio <= c.max) "
			+ "OR (c.max IS NULL AND :valeurRatio > c.min))")
